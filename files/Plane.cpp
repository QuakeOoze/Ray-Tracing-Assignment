...
*/
bool Plane::isInside(glm::vec3 pt, glm::vec3 n)
{
    ...
    float crossA = (glm::dot(glm::cross(uA, vA), n));
    float crossB = (glm::dot(glm::cross(uB, vB), n));
    float crossC = (glm::dot(glm::cross(uC, vC), n));
    float crossD = (glm::dot(glm::cross(uD, vD), n));
